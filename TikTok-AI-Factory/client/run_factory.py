...
def main():
    ...
    # ================================================================
    # AUTO-UPDATE CHECK — 启动时检查新版本
    # ================================================================
    skip_update = "--skip-update" in sys.argv
    if skip_update:
        sys.argv.remove("--skip-update")
    dev_mode = os.getenv("TIKTOK_FACTORY_DEV_MODE", "").lower() in ("true", "1", "yes")
    skip_update_env = os.getenv("TIKTOK_FACTORY_SKIP_UPDATE", "").lower() in ("true", "1", "yes")

    if not skip_update and not dev_mode and not skip_update_env:
        try:
