...
def _get_case_browser():
    global _CASE_BROWSER
    if _CASE_BROWSER is None:
        try:
            from app.gui.case_browser import CaseBrowser
            _CASE_BROWSER = CaseBrowser
        except ImportError:
            _CASE_BROWSER = False
    return _CASE_BROWSER
...
class TikTokFactoryGUI(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Tab 6: Case Library Browser
        CBTab = _get_case_browser()
        if CBTab and CBTab is not False:
            self.case_browser = CBTab()
            self.main_tabs.addTab(self.case_browser, "📚 案例库")
        else:
...
def main():
    ...
    # First-run API config wizard
    try:
        from app.gui.api_wizard import show_wizard_if_needed
        show_wizard_if_needed(window)
    except Exception:
        pass
